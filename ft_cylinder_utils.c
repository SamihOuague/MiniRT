...
/*   Updated: 2024/10/23 11:59:46 by souaguen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_cy_normal(t_cylinder cy, t_ray *ray)
{
	...
	t = ft_dot(cy.axis, ft_sub(hit, ft_sub(cy.origin, (*ray).from)));
	normal = ft_sub(ft_sub(hit, ft_product(cy.axis, t)), ft_sub(cy.origin, (*ray).from));
...
int	ft_caps_intersection(t_cylinder cy, t_ray *ray)
{
	...
	if (ft_dot(hit, hit) < pow(cy.radius, 2))
		return (1);
...
int	ft_cylinder_intersection(void *cylinder, t_ray *ray)
{
	...
	equation.y = ft_dot(cross_d, ft_cross_product(ft_sub(cy.origin, (*ray).from), cy.axis));
	equation.z = (equation.x * pow(cy.radius, 2)) - (ft_dot(cy.axis, cy.axis) * pow(ft_dot(ft_sub(cy.origin, (*ray).from), cross_d), 2));	
	caps_ray = *ray;
	...
	if (t[0] > t[1])
		t[0] = t[1];
	t[1] = ft_dot(cy.axis, ft_sub(ft_product((*ray).direction, t[0]), ft_sub(ft_sub(cy.origin, (*ray).from), (*ray).from)));
	if (t[0] < 0 || t[1] < 0 || t[1] >= cy.height)
		return (ft_caps_intersection(cy, ray));
